...
func processFsMessage(myFsServer *FileSystemServer, msg []byte, n int){
  ...
  if myFsMsg.ID == "PUT_START"{
   fmt.Println("received PUT_START shubham1")
   myFsServer.PutProgressFiles[myFsMsg.Num] = 1
  }else if myFsMsg.ID == "PUT"{
    fmt.Println("received PUT shubham1")
    ...
    fmt.Println("received PUT_END shubham1")
    delete(myFsServer.PutProgressFiles, myFsMsg.Num)
  }else if myFsMsg.ID == "DELETE"{
    fmt.Println("received DELETE shubham1")
...
/*
  fsMsg.messageType = msgType
  fsMsg.fromHost = myFsServer.hostname
  fsMsg.replicaIds = replicaIds
  fsMsg.filename = sdfsfilename
*/

  fmt.Println("shubham ReplicaIds:")
...
func get(myFsServer *FileSystemServer, sdfsfilename string, localfilename string){
  ...
  if !present{
    fmt.Println("Sorry! Can't GET a file which is not present :-(")
    return
  }*/
  ...
  fmt.Println("shubham1: " + sdfsfilename)
...
func store(){
  ...
  fmt.Printf("%v\n", string(output))
/*
  cmd := exec.Command("ls -lrht","/home/ss77/files")
  fmt.Println("Store command:")
  fmt.Println(cmd)
  files, err := cmd.Output()
  if err != nil{
    log.Println("Unable to receive output while running ls: ",err)
    return
  }
  fmt.Printf("%v",string(files))
*/
}
