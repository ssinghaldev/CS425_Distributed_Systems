...
//InitialMessage which is given by INTRODUCER to the new joinee
...
//General Message format - used for various messages - join/leave/heartbeat etc
type Message struct {
  MessageType string
  Hostname string
  Timestamp string
...
//Initialising membership list of a server
func InitMembershipList(hostname string) map[string]*MembershipElement{
  ...
  for idx, hostname := range ALL_HOSTS["all_hosts_info"]{
    membershipList[hostname] = &MembershipElement{strconv.Itoa(idx+1), hostname,
                                                  HOST_TO_IP[hostname], time.Now().Format("15:04:05.000000") ,
                                                  "INIT"}
  }
  return membershipList
}

//Initialising server info
//Used for getting membershiplist and neighbourlist
func InitMyServer(hostname string) *ServerInfo{
  ...
  server.port = DEFAULT_PORT_TO_LISTEN
...
//A goroutine for listening various commands
//Spawns another thread to process each message
...
// Sending JOIN request to INTRODUCER
// if INTRODUCER, disseminating info to all nodes
...
//sending heartbeats periodically to the neighbours
...
//Monitoring hearbeats of my neighbours
//Disseminating failure information to neigbours if didn't receive heartbeat
//within a interval
...
//Function which processes different types of messages
...
//checking if current server is INTRODUCER
func AmIIntroducer(myServer *ServerInfo) bool{
  return myServer.hostname == INTRODUCER
}

//Bunch of helper functions for Marshalling and Unmarshalling messages 
